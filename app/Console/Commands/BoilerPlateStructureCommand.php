...
class BoilerPlateStructureCommand extends Command
{
...
class {$name}Repository implements {$name}Interface
{
    ...
    \DB::beginTransaction();
        try {
            \${$smallFirstChar}Store = {$name}::create(\$data);
            \DB::commit();
            return response()->json(['success' => 'Successfully Created {$name}', '{$smallFirstChar}' => \${$smallFirstChar}Store], 200);
        } catch (\Exception \$e) {
            \DB::rollBack();
    ...
    public function update{$name}(\$id, array \$data)
    {
        ...
        \DB::beginTransaction();
        try {
            \${$smallFirstChar}Update = {$name}::findOrFail(\$id);
            \${$smallFirstChar}Update->update(\$data);
            \DB::commit();
            return response()->json([
                'success' => 'Successfully Updated {$name}',
                '{$smallFirstChar}' => \${$smallFirstChar}Update
            ], 200);
        } catch (\Exception \$e) {
            \DB::rollBack();
    ...
    public function delete{$name}(\$id)
    {
        \DB::beginTransaction();
        try {
            \${$smallFirstChar} = {$name}::findOrFail(\$id);
            \${$smallFirstChar}->delete();
            \DB::commit();
            return response()->json([
...
class {$name}Service
{
    protected \${$smallFirstChar}Repository;

    public function __construct({$name}Interface \${$smallFirstChar}Repository)
    {
        \$this->{$smallFirstChar}Repository = \${$smallFirstChar}Repository;
    }

    public function getAll{$name}s()
    {
        return \$this->{$smallFirstChar}Repository->getAll{$name}s();
    }

    public function get{$name}ById(\$id)
    {
        return \$this->{$smallFirstChar}Repository->get{$name}ById(\$id);
    }

    public function create{$name}(array \$data)
    {
        return \$this->{$smallFirstChar}Repository->create{$name}(\$data);
    }

    public function update{$name}(\$id, array \$data)
    {
        return \$this->{$smallFirstChar}Repository->update{$name}(\$id, \$data);
    }

    public function delete{$name}(\$id)
    {
        return \$this->{$smallFirstChar}Repository->delete{$name}(\$id);
    }
}
";
